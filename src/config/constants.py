...
RANDOM_FRESHNESS_MAX_BOOST = 0.05
RANDOM_FRESHNESS_FULL_DAYS = 14.0
RANDOM_FRESHNESS_DECAY_DAYS = 90.0
MAX_FEED_ITEMS = 30

# Discord attachment/proxy URLs carry expiring signatures. Preserve existing
...
DISCORD_DEAD_LINK_WEBHOOK_URL = os.getenv("DISCORD_DEAD_LINK_WEBHOOK_URL", "").strip()
DISCORD_REVIVAL_WEBHOOK_URL = os.getenv("DISCORD_REVIVAL_WEBHOOK_URL", "").strip()
DISCORD_EMBED_WAIT_SECONDS = float(os.getenv("DISCORD_EMBED_WAIT_SECONDS", "15"))
DISCORD_EMBED_POLL_INTERVAL_SECONDS = float(os.getenv("DISCORD_EMBED_POLL_INTERVAL_SECONDS", "2"))
RECOVERY_INTERVAL_SECONDS = int(os.getenv("RECOVERY_INTERVAL_SECONDS", str(75 * 60)))
# Weekly ELO snapshots (Monday KST, sparse). Same 6h cadence as tsuki-bot.
BIAS_SNAPSHOT_INTERVAL_SECONDS = int(os.getenv("BIAS_SNAPSHOT_INTERVAL_SECONDS", str(6 * 60 * 60)))
