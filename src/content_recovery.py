...
class PendingRecovery:
    """An uploaded replacement awaiting a database commit."""
...
def parse_single_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--auth-env",
        choices=("USER_AUTH",),
        help="Environment variable containing read-only Discord credentials (default: USER_AUTH)",
    )
    parser.add_argument(
        "--max-pages",
        type=int,
...
def auth_header(auth_env: str | None) -> tuple[str, str]:
    """Load the read-only Discord user authorization without logging it."""

    selected_env = auth_env or os.getenv("RECOVERY_AUTH_ENV", "USER_AUTH")
    if selected_env != "USER_AUTH":
        raise ValueError("Discord access in the web app is read-only and only supports USER_AUTH")
    value = os.getenv("USER_AUTH", "").strip()
    if not value:
        raise RuntimeError("USER_AUTH is empty or not present in .env")
    return value, "USER_AUTH"


class DiscordReadClient:
    def __init__(self, authorization: str | None = None):
        self.session = requests.Session()
        self.session.headers.update({"User-Agent": "hanni-content-recovery/1.0"})
        if authorization:
        ...
        for attempt in range(6):
            ...
            try:
                ...
                if response.status_code == 401:
                    raise RuntimeError("Discord rejected the read-only credentials (401); check USER_AUTH in .env")
        ...
        self.session.headers.update(
            {
                ...
                "User-Agent": "hanni-content-recovery/1.0",
            }
        )
    ...
    def verify_direct_url(self, media_url: str, attempts: int = 5) -> None:
        ...
        for attempt in range(attempts):
            ...
            try:
                ...
                content_length = int(content_length_header) if content_length_header and content_length_header.isdigit() else -1
                if (
                    200 <= response.status_code < 300
                    and detect_extension(content_type, first_bytes)
                    and not is_imgur_deleted_placeholder(content_type, first_bytes, content_length)
                ):
                    return
            finally:
                response.close()
...
def recover_direct_imgur(
    media_client: DiscordReadClient, media_id: str, output_dir: Path, force: bool
) -> DownloadedMedia | None:
...
def history_search_and_recover(
    discord_client: DiscordReadClient,
    media_client: DiscordReadClient,
    channel_id: str,
    target_url: str,
    media_id: str,
...
def recover_content(
    ...
    discord_client = DiscordReadClient(authorization)
    try:
...
def fetch_candidates(
    ...
    query = f"""
        ...
        one_candidate_per_url AS (
            ...
            ORDER BY
                url,
                recovery_generation DESC,
                initial_reaction_count DESC,
                num_reports DESC,
                uploaded_date ASC,
                content_link_id ASC
        )
...
def mark_recovery_exhausted(connection: psycopg.Connection[Any], candidate: Candidate) -> bool:
    """Stop retrying every dead row with this URL after the configured failures."""
...
def apply_success(
    ...
    with connection.transaction():
        with connection.cursor() as cursor:
            ...
            if result is None:
                raise RuntimeError(
                    f"Database rows changed, are no longer dead, or disappeared for url={candidate.url}; "
    ...
    with tempfile.TemporaryDirectory(prefix=f"content-recovery-{candidate.content_link_id}-") as temporary_dir:
        ...
        try:
            ...
            return PendingRecovery(
                ...
                uploaded=uploaded,
                batch_id=batch_id,
                recovery_method=downloaded.recovery_method,
                downloaded_size=downloaded.size,
                trimmed_size=trimmed.stat().st_size,
                trimmed_sha256=trimmed_sha256,
...
def finalize_pending_recovery(connection: psycopg.Connection[Any], pending: PendingRecovery) -> None:
    ...
    try:
        apply_success(
            ...
            pending.batch_id,
            pending.recovery_method,
            pending.downloaded_size,
            pending.trimmed_size,
            pending.trimmed_sha256,
            pending.uploaded.media_id,
            pending.replacement_generation,
        )
        print(f"UPDATED {pending.candidate.content_link_id}: {pending.uploaded.url}")
    except Exception as error:
        update_recovery_item(
            connection,
            pending.batch_id,
...
def parse_batch_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--role-id",
        default=None,
        help="Only recover links for this role (default: all roles)",
    )
    parser.add_argument(
        "--limit",
        type=int,
