...
def _board_image(entry_role_id: str, image_url: str | None) -> str | None:
    """Vendored portraits first (they're the harvested Discord shots, slimmed);
    Discord-proxied portraits cover the gaps."""
...
async def photos_page(request: Request) -> HTMLResponse:
    response = templates.TemplateResponse(
        request=request,
        name="photos.html",
        context={
